	...
	public function undo_delete()
	{
		...
		if ($this->Trash->undo_deleted_list($users)) {
             foreach ($users as $user_) {
                save_user_log($this->lang->line("user_log_restore_user")." ".$this->User->get_info($user_)->first_name );
            }
	...
	*/
	public function delete()
	{
        ...
        foreach ($users_to_delete as $deleted_user_id) {
                 $deleted_users[]=$this->User->get_info($deleted_user_id)->first_name ;
            }
		if ($this->Trash->delete_list($users_to_delete)) {
             foreach ($deleted_users as $deleted_user) {
                save_user_log($this->lang->line("user_log_permanently_delete_user")." ". $deleted_user);
            }
