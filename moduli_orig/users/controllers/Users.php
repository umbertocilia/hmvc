...
class Users extends Secure_area implements iData_controller
{
	...
	*/
	...
	public function save_profile_pic($user_id=-1)
	{
			...
			if ($this->User->update_user_info($userinfo_data,$user_id)) {
				...
				fclose( $fp );
				save_user_log($this->lang->line("user_log_profiles_avatar_updated")." ".$this->User->get_info($user_id)->first_name );
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if($user_id==-1 && $usermailcount!=0){
		       ...
		       if ($this->User->save($userinfo_data,$userlog_data,$permission_data,$user_id)) {
			    //New user
			        if ($user_id==-1) {
                        save_user_log($this->lang->line("user_log_new_user")." ".$this->input->post('first_name') );
                    ...
                    save_user_log($this->lang->line("user_log_profile_updated_for_user")." ".$this->User->get_info($user_id)->first_name );
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($password_match==0) {
				...
				if ($this->User->update_password( $userlog_data,$userinfo_data,$user_id)) {
                    save_user_log($this->lang->line("user_log_password_updated_for_user")." ".$this->User->get_info($user_id)->first_name );
	...
	*/
	public function export()
	{
        ...
        if($data_row->active==0 and $data_row->deleted==0)
        ...
        if($data_row->deleted==1)
	    $value =  $this->lang->line('profiles_deleted') ;	
        else
	    $value =  "";	
        $value = str_replace( '"' , '""' , $value );
	    $value = '"' . $value . '"' . ",";
	...
	*/
	public function delete()
	{
		...
		if ($this->User->delete_list($users_to_delete)) {
            foreach ($users_to_delete as $deleted_user_id) {
                save_user_log($this->lang->line("user_log_delete_user")." ".$this->User->get_info($deleted_user_id)->first_name );
            }
