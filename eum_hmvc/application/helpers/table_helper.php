...
//User logs
function get_user_log_manage_table($people,$controller)
{
	$CI =& get_instance();
	$table='<table class="tablesorter" id="sortable_table">';
	
	$headers = array('<input type="checkbox" id="select_all" />', 
	$CI->lang->line('user_log_user'),
	$CI->lang->line('user_log_ip_address'),
	$CI->lang->line('user_log_log_time'),
	$CI->lang->line('user_log_log_message'),
	...
	$table.='</tr></thead><tbody>';
	$table.=get_user_log_manage_table_data_rows($people,$controller);
	$table.='</tbody></table>';
	return $table;
}
...
Gets the html data rows for the people.
*/
function get_user_log_manage_table_data_rows($people,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	
	foreach ($people->result() as $data_row) {
		$table_data_rows.=get_user_log_data_row($data_row,$controller);
	}
	
	if ($people->num_rows()==0) {
		$table_data_rows.="<tr><td colspan='10'><div class='warning_message' style='padding:7px;'>".$CI->lang->line('user_log_no_log_to_display')."</div></tr></tr>";
	}
	
	return $table_data_rows;
}

function get_user_log_data_row($data_row,$controller)
{
	$CI =& get_instance();
	$controller_path=$CI->router->fetch_module()."/".$CI->router->fetch_class();
	$width = $controller->get_form_width();
   $table_data_row='<tr>';	
	    $table_data_row.="<td width='5%'><input type='checkbox' id='page_$data_row->user_log_id' value='".$data_row->user_log_id."'/></td>";
	// Apply the xss_clean() of "security" library, which filtered data from passing through <script> tag.
	$table_data_row.='<td width="15%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->first_name)),10).'</td>';
	$table_data_row.='<td width="15%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->	ip_address)),10).'</td>';
	$table_data_row.='<td width="20%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->log_time)),50).'</td>';
    $table_data_row.='<td width="40%">'.character_limiter(html_escape($CI->security->xss_clean( $data_row->log_message)),50).'</td>';
	 
 
  
	$table_data_row.='<td width="10%">'.anchor($controller_path."/view/$data_row->user_log_id/width:360", $CI->lang->line('icon_view'),array('class'=>'thickbox','title'=>$CI->lang->line('common_view'))).'</td>';	
...
//Active session
function get_active_session_manage_table($people,$controller)
{
	$CI =& get_instance();
	$table='<table class="tablesorter" id="sortable_table">';
	...
	$table.='</tr></thead><tbody>';
	$table.=get_active_session_manage_table_data_rows($people,$controller);
	$table.='</tbody></table>';
	return $table;
}
...
Gets the html data rows for the people.
*/
function get_active_session_manage_table_data_rows($people,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	
	foreach ($people->result() as $data_row) {
		$table_data_rows.=get_active_session_data_row($data_row,$controller);
	}
...
function get_active_session_data_row($data_row,$controller)
{
	$CI =& get_instance();
	$controller_path=$CI->router->fetch_module()."/".$CI->router->fetch_class();
	$width = $controller->get_form_width();
   $table_data_row='<tr>';	
	    $table_data_row.="<td width='5%'><input type='checkbox' id='page_$data_row->actve_sessions_id' value='".$data_row->actve_sessions_id."'/></td>";
    $table_data_row.='<td width="10%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->first_name)),10).'</td>';
	$table_data_row.='<td width="15%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->	ip_address)),10).'</td>';
	$table_data_row.='<td width="20%">'.character_limiter(html_escape($CI->security->xss_clean($data_row->log_time)),50).'</td>';

  
	$table_data_row.='<td width="10%">'.anchor($controller_path."/view/$data_row->actve_sessions_id/width:360", $CI->lang->line('icon_view'),array('class'=>'thickbox','title'=>$CI->lang->line('common_view'))).'</td>';	
