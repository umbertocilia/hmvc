...
<section class="content">
	...
	<?php if(!is_writable_r($upload_path)){?>
	    ...
	    <p>The uploads directory is not writable. Please check the permissions on that directory<i> (change the folder permissions to CHMOD 757 on the 'uploads' folder and all sub folders).</i> </p>
		</div>
		 <?php } ?>
		 <br>
		   <?php if(!is_writable_r($captcha)){?>
	    <div  class="warning"><h6>Warning</h6>
	    <p>The captcha directory is not writable. Please check the permissions on that directory<i>(change the folder permissions to CHMOD 757 on the 'uploads' folder and all sub folders)</i> </p>
		</div>
		 <?php } ?>
		  <br>
		   <?php if(!is_writable_r($cache)){?>
	    <div  class="warning"><h6>Warning</h6>
	    <p>The cache(application/cache) directory is not writable. Please check the permissions on that directory<i>(change the folder permissions to CHMOD 757 on the 'uploads' folder and all sub folders)</i> </p>
		</div>
		 <?php } ?>
	<br>
    <?php if(is_writable($db_config_path)){?>
		...
		<br>
          ...
          <label for="hostname" class="col-sm-4">Hostname</label>	<div class="col-sm-8">
     	<input type="text" id="hostname" value="" class="input_text form-control" name="hostname"  />
	    </div>
         </div>
       ...
       <div class="form-group">	
        ...
        <label for="database"   class="col-sm-4">Database Name</label>
         ...
         <label for="dbprefix"   class="col-sm-4">Database Prefix</label>
		  <div class="col-sm-8">
     	 <input type="text" id="dbprefix" value="eum_" class="input_text form-control" name="dbprefix" />
	    </div>
         </div>
